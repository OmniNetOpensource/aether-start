...
export default function renderMonitorPlugin({ types: t }) {
  ...
  function addImport(programPath, name) {
    ...
    if (existing) {
      ...
      if (spec) return;
      existing.node.specifiers.push(t.importSpecifier(t.identifier(name), t.identifier(name)));
      return;
    }
  ...
  return {
    ...
    visitor: {
      ...
      FunctionDeclaration(path, state) {
        if (!state.opts?.enabled) return;
        const file = state.filename || state.file?.opts?.filename || '';
        if (!file || file.includes('node_modules') || file.includes('render-monitor')) return;
