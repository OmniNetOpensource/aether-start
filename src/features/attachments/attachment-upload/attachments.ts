 ...
 * 聊天附件模块
 *
 * 负责图片附件的校验、上传及状态管理�? * 上传后的资源存储�?Cloudflare R2，通过 /api/upload-attachment 接口完成�? */
...
/** 生成唯一附件 ID，优先用 crypto.randomUUID，否则用时间�?随机数兜�?*/
...
/**
 * 校验文件是否为可上传的图�? * @returns 校验失败时返回错误提示文案，通过时返�?null
 */
const buildValidationMessage = (file: File) => {
  ...
  if (file.size > MAX_IMAGE_SIZE) {
    return `图片�?{file.name}」超�?${(MAX_IMAGE_SIZE / (1024 * 1024)).toFixed(0)}MB 限制。`;
  }
...
/**
 * �?blob 上传�?/api/upload-attachment，存�?R2
 * @returns 服务端返回的 storageKey �?url
 */
 ...
 * @returns 上传成功返回 Attachment，失败返�?null
 */
