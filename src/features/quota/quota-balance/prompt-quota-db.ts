...
export const getOrCreateUserQuota = async (
  ...
  const row = await db
    .prepare(
      'SELECT user_id, balance, created_at, updated_at FROM user_prompt_quota WHERE user_id = ?1',
    )
    .bind(userId)
    .first();
...
export const adminListRedeemCodes = async (
  ...
  const nextCursor: RedeemCodeCursor =
    results.length === input.limit && last ? { created_at: last.created_at, id: last.id } : null;
