...
const buildBranchedMessages = (): Message[] => {
  const root = addMessage(
    createEmptyMessageState(),
    "user",
    [textBlock("q1")],
    "2024-01-01",
  );
  const firstAssistant = addMessage(
    root,
    "assistant",
    [textBlock("a1")],
    "2024-01-02",
...
describe("useChatSessionStore", () => {
  ...
  it("appends streamed text to the latest assistant message", () => {
    ...
    useChatSessionStore
      .getState()
      .initializeTree(linear.messages, linear.currentPath);
  ...
  it("creates an assistant message when appending with no assistant at path end", () => {
    const linear = createLinearMessages([
      { role: "user", blocks: [textBlock("question")] },
    ]);
    useChatSessionStore
      .getState()
      .initializeTree(linear.messages, linear.currentPath);
  ...
  it("selects a message and rebuilds the active path through its descendants", () => {
    ...
    useChatSessionStore
      .getState()
      .initializeTree(state.messages, state.currentPath);
  ...
  it("tracks artifact streaming state and history selection", () => {
    useChatSessionStore.getState().setConversationId("conv-1");
    useChatSessionStore.getState().startArtifact("artifact-1");
    useChatSessionStore.getState().updateArtifactTitle("artifact-1", "Landing");
    useChatSessionStore
      .getState()
      .updateArtifactLanguage("artifact-1", "react");
    useChatSessionStore
      .getState()
      .appendArtifactCode("artifact-1", "export default function App(){");
    useChatSessionStore
      .getState()
      .appendArtifactCode(
        "artifact-1",
    ...
    useChatSessionStore.getState().completeArtifact("artifact-1");
    ...
    expect(useChatSessionStore.getState().artifacts[0]).toMatchObject({
      id: "artifact-1",
    ...
    expect(useChatSessionStore.getState().artifacts[0].code).toContain(
      'className="p-4"',
    );
  });

  it("marks failed artifacts and keeps the code view active", () => {
    useChatSessionStore.getState().setConversationId("conv-1");
    useChatSessionStore.getState().startArtifact("artifact-2");
    useChatSessionStore.getState().appendArtifactCode("artifact-2", "<main>");
    useChatSessionStore
      .getState()
      .failArtifact("artifact-2", "Tool execution failed");
