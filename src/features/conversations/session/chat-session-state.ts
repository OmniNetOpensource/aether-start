...
export type ChatSessionActions = {
  ...
  updateArtifactDeployment: (artifactId: string, url: string, deployedAt: string) => void;
...
const getTreeState = (state: ChatSessionState): TreeSnapshot => ({
...
export const createInitialChatSessionState = (
  ...
  return {
    ...
    pageTitle: 'Aether',
    ...selection,
    currentModelId: selection.currentModelId || initialModelId,
...
export const createChatSessionActions = (
  ...
  setState: Dispatch<SetStateAction<ChatSessionState>>,
): ChatSessionActions => {
  const set = (
    update:
      | Partial<ChatSessionState>
      | ((state: ChatSessionState) => Partial<ChatSessionState> | ChatSessionState),
  ) => {
    setState((state) => ({
      ...state,
  ...
  return {
    ...
    selectMessage: (messageId) => {
      ...
      let nextState = getTreeState(state);
    ...
    navigateBranch: (messageId, depth, direction) => {
      ...
      const nextState = switchBranch(getTreeState(state), depth, targetId);
    ...
    updateArtifactDeployment: (artifactId, url, deployedAt) =>
      set((state) => ({
        artifacts: state.artifacts.map((artifact) =>
          artifact.id === artifactId
            ? {
    ...
    setCurrentFetchProvider: (provider) => set({ currentFetchProvider: provider }),
    ...
    addMessage: (role, blocks, createdAt) => {
      const result = addMessage(getTreeState(get()), role, blocks, createdAt);
      set({
    ...
    editMessage: (depth, messageId, blocks) => {
      const result = editMessage(getTreeState(get()), depth, messageId, blocks);
      if (!result) {
        return null;
      }
