  ...
  try {
    ...
    if ("jobId" in result && result.jobId) {
      ...
      for (let i = 1; i <= YOUTUBE_MAX_POLLS; i++) {
        ...
        if (job.status === "completed" && job.result) {
          ...
          const sizeKB = (
            new TextEncoder().encode(text).byteLength / 1024
          ).toFixed(1);
          await emitProgress(onProgress, {
            stage: "complete",
    ...
    const text =
      typeof transcript.content === "string"
        ? transcript.content
        : JSON.stringify(transcript.content);
    const sizeKB = (new TextEncoder().encode(text).byteLength / 1024).toFixed(
      1,
    );
    await emitProgress(onProgress, {
      stage: "complete",
...
const fetchUrl: ToolHandler = async (args, onProgress, signal) => {
  ...
  if (response_type === "image") {
    if (isDirectImageUrl(url)) {
      return enqueueFetchUrlCall(() =>
        fetchDirectImage(url, onProgress, signal),
      );
    } else {
      return enqueueFetchUrlCall(() =>
  ...
  return enqueueFetchUrlCall(() =>
    performFetchUrl(url, onProgress, signal),
  );
};
