...
const parseReactCode = (code: string) => {
  if (/\bimport\s+/.test(code)) {
    throw new Error(
      "React artifacts must be self-contained and cannot use import statements",
    );
  }
...
export const parseRenderArgs = (args: unknown): RenderArgs => {
  ...
  if (title.length > TITLE_MAX_LENGTH) {
    throw new Error(
      `render title must be ${TITLE_MAX_LENGTH} characters or fewer`,
    );
  }
  ...
  if (code.length > CODE_MAX_LENGTH) {
    throw new Error(
      `render code must be ${CODE_MAX_LENGTH} characters or fewer`,
    );
  }
