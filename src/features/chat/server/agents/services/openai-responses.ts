...
const convertToolsToOpenAIResponses = (
  tools: ChatTool[],
        ...
        content:
          contentItems.length === 0
            ...
            : contentItems.length === 1 &&
                contentItems[0]?.type === "input_text"
              ? contentItems[0].text
              : contentItems,
      };
...
export function formatOpenAIResponsesToolContinuation(
  ...
  const functionCallItems: OpenAI.Responses.ResponseFunctionToolCall[] =
  ...
  const functionCallOutputItems: OpenAI.Responses.ResponseInputItem.FunctionCallOutput[] =
    toolResults.map((toolResult, index) => ({
      ...
      call_id:
        toolResult.id || pendingToolCalls[index]?.id || `tool_${index + 1}`,
      output: toolResult.result,
    }));

  const toolResultImageContent: OpenAI.Responses.ResponseInputContent[] =
...
export class OpenAIResponsesChatProvider {
  ...
  constructor(config: OpenAIResponsesChatProviderConfig) {
    ...
    this.openaiTools =
      config.tools.length > 0
        ? convertToolsToOpenAIResponses(config.tools)
        : undefined;
  }
  ...
  async *run(
    ...
    const streamParams: OpenAI.Responses.ResponseCreateParamsStreaming = this
      .openaiTools
      ? {
          model: this.model,
          input: messages,
      ...
      for await (const event of stream) {
        ...
        if (
          ...
          if (
            ...
            !renderParsers.has(event.output_index)
          ) {
    ...
    const pendingToolCalls: PendingToolInvocation[] = [
      ...toolCallsByOutputIndex.entries(),
    ]
      .sort(([a], [b]) => a - b)
      .map(([index, toolCall]) => {
    ...
    for (const [index, toolCall] of [...toolCallsByOutputIndex.entries()].sort(
      ...
      for (const artifactEvent of parser.finalize(
        pendingToolCalls.find(
          (item) => item.id === (toolCall.callId || `tool_${index + 1}`),
        )?.args ?? {},
      )) {
        yield artifactEvent;
      }
