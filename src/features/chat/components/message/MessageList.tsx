...
export function MessageList({ className, listClassName }: MessageListProps = {}) {
  ...
  const canNext =
    hasMessages && currentPath.length > 1 && currentIdx >= 0 && currentIdx < currentPath.length - 1;
  ...
  const handlePrev = () => {
    ...
    const el = container.querySelector<HTMLElement>(`[data-message-id="${prevId}"]`);
  ...
  const handleNext = () => {
    ...
    const el = container.querySelector<HTMLElement>(`[data-message-id="${nextId}"]`);
    el?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
