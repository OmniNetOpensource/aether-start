...
describe('MessageItem', () => {
  ...
  it('renders peeking attachments above the user bubble when text exists', async () => {
    ...
    const bubble = container.querySelector('.rounded-lg.bg-\\(--surface-muted\\)')
  ...
  it('keeps the bubble and preview interaction when the message only has attachments', async () => {
    ...
    expect(stack).not.toBeNull()
    expect(container.querySelector('.rounded-lg.bg-\\(--surface-muted\\)')).not.toBeNull()
    ...
    expect(
      document.body.querySelector('img[src="https://example.com/solo-full.png"]'),
    ).not.toBeNull()
  })
})
