...
export function MessageEditor({ messageId, depth }: MessageEditorProps) {
  ...
  useEffect(() => {
    ...
    const paddingBottom = Number.isFinite(paddingBottomValue) ? paddingBottomValue : 0;
    const maxLines = 5;
  ...
  const sendDisabled =
    isBusy || uploading || (!hasText && !hasQuotes && !hasAttachments) || !currentModelId;
  ...
  const handleRemoveAttachment = (id: string) => {
    updateEditAttachments(editedAttachments.filter((a) => a.id !== id));
  };
