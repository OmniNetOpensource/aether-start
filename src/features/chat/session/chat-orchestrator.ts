 ...
 * 职责�? * - 发起聊天请求（startChatRequest�? * - 取消正在进行�?AI 回复（cancelAnswering�? * - 恢复正在进行的对话流（resumeRunningConversation�? * - 消费 SSE 流并分发事件到消息树
 *
...
/**
 * 已处理的最�?eventId，用于去重和断点续传�? * 服务端事件带 eventId，客户端只处�?eventId > lastEventId 的事件�? */
let lastEventId = 0;
...
/**
 * 当前活跃�?AbortController�? * 同一时刻只允许一个请求在跑，新请求会 abort 掉旧的�? */
...
/**
 * 解析 Agent �?base URL�? * 根据当前页面�?protocol �?host 拼接，SSR 时回退�?localhost:3000�? */
...
/**
 * 生成唯一 ID�? * 优先�?crypto.randomUUID()，不支持时用 prefix + 时间�?+ 随机数�? */
const generateId = (prefix = 'id') =>
  typeof crypto !== 'undefined' && crypto.randomUUID
    ? crypto.randomUUID()
    : `${prefix}_${Date.now()}_${Math.random().toString(16).slice(2)}`;

/** 判断是否�?AbortController 触发的取消错误，用于静默忽略用户主动停止 */
const isAbortError = (error: unknown) =>
  (error instanceof DOMException && error.name === 'AbortError') ||
  (error instanceof Error && error.name === 'AbortError');

/**
 * 流结束后的收尾逻辑�? * 异常结束时尝试自动重连；正常 idle 或无法重连时清理重连状态�? */
...
/**
 * 处理单条 SSE 消息�? * @param event - SSE �?event 字段（如 chat_event、chat_started、chat_finished 等）
 * @param raw - data 字段的原�?JSON 字符�? */
const handleSSEMessage = (event: string, raw: string) => {
  ...
  switch (event) {
    case 'chat_event': {
      /* 单条聊天事件：需�?eventId 且大�?lastEventId 才处理，避免重复 */
    ...
    case 'chat_finished':
      ...
      return;
    case 'sync_response': {
      /* 断点续传：服务端返回已有事件列表，按 eventId 去重后依次应�?*/
    ...
    case 'conversation_update':
      /* 对话元数据更新（如标题），转�?conversation_updated 事件应用 */
      if (
...
/**
 * 消费 SSE 流式响应�? * �?\n\n 分割事件块，解析 event: �?data: 行，调用 handleSSEMessage 处理�? * 支持 signal 中断；结束时调用 reader.cancel 释放资源�? */
...
/**
 * 探测指定对话�?Agent 状态�? * GET /agents/chat-agent/:conversationId，返�?idle | running | completed | aborted | error�? * 404 视为 idle�? */
...
/**
 * 发起一次聊天请求�? *
 * 流程�? * 1. 校验 status �?idle、已选模�? * 2. 若无 conversationId，创建新对话并导航到 /app/c/:id
 * 3. 构建 body（idempotencyKey、model、promptId、conversationHistory、treeSnapshot�? * 4. 取消之前的请求，发起 POST /agents/chat-agent/:conversationId/chat
 * 5. 处理 409（busy）�?02（配额超限）
 * 6. 消费 SSE 流，结束�?finalizeStream
 *
 * 异常：AbortError 静默忽略；TypeError（如网络错误）调�?finalizeStream；其他恢�?idle�? */
export const startChatRequest = async () => {
  ...
  activeController?.abort(); /* 取消之前的请求，保证同一时刻只有一个在�?*/
...
/**
 * 取消订阅流式输出�? * abort 本地 activeController，将 status 设为 idle�? */
...
/**
 * 取消正在进行�?AI 回复�? * 集成 cancelStreamSubscription �?POST /abort 通知服务端�? */
 ...
 * 流程�? * 1. 调用 checkAgentStatus，若不为 running 则直接设�?idle 返回
 * 2. 创建�?AbortController，通过 activeController �?cancelStreamSubscription 联动
 * 3. POST /agents/chat-agent/:conversationId/events，body �?{ lastEventId }
 * 4. 消费返回�?SSE 流（sync_response + 后续 chat_event�? * 5. 结束�?finalizeStream
 *
 * 取消方式：对话页卸载时调�?cancelStreamSubscription 即可 abort
 */
