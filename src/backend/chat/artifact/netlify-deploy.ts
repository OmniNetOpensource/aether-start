...
export const deployArtifactToNetlify = async ({
  artifactId,
  html,
  userId,
}: {
  ...
  if (!artifactRow || typeof artifactRow.id !== 'string') {
    log('NETLIFY_DEPLOY', 'artifact_not_found', { artifactId });
    throw new Error('Artifact not found');
  }

  log('NETLIFY_DEPLOY', 'request_start', { artifactId });

  try {
    const result = await deployArtifactAndSaveDeployment({
      artifactId,
      html,
      deploy: (html) => deployHtmlToNetlify({ html, netlifyToken: NETIFY_TOKEN }),
      persist: async ({ artifactId, deployUrl, deployedAt }) => {
        await DB.prepare(
          ...
          .bind(deployUrl, deployedAt, deployedAt, userId, artifactId)
          .run();
      },
    });
    ...
    log('NETLIFY_DEPLOY', 'deploy_error', { artifactId, error });
    throw error;
  }
};
