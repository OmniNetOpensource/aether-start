...
import { Button, buttonVariants } from '@/frontend/design-system/button';
import { cn } from '@/shared/core/utils';
import { reportClientError } from '@/frontend/browser/report-client-error';

type Props = { children: ReactNode };

type State = { status: 'ok' } | { status: 'error'; value: Error } | { status: 'unknown' };

function AppErrorPage({ error }: { error: Error | null }) {
  const detail = error?.message ?? '发生了未知错误';
...
export class AppErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    if (this.state.status === 'error') return <AppErrorPage error={this.state.value} />;
