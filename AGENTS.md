...
Aether 本地服务的生命周期由代理自行决定。需要开发或验收时，禁止询问或要求用户启动、重启、停止本地服务；代理应自行检查服务状态，并按任务需要决定启动、重启、停止、保持运行或改用空闲端口。操作前必须确认端口和进程归属，不得干扰不属于本仓库的服务；只能停止能够确认属于本仓库的进程。测试需要本地服务时直接启动并测试，不再向用户询问是否启动或是否测试。本规则取代其他要求用户启动本地服务或禁止代理管理本地服务的仓库内说明。即使服务已由用户启动，也不得影响用户明确管理的其他常驻服务。
